...
def main(algorithm, start, end):
    ...
    node1=(point_start[1],point_start[0])
    node2=(point_end[1],point_end[0])
    ...
    if algorithm == 'busqueda_amplitud':
        ...
        resultado_final = reconstruct_path(busqueda, start_node, end_node)
        return resultado_final
    elif algorithm == 'suavizado':
        G = ox.shortest_path_graph(place_name, node1, node2, network_type="drive", simplify=False)
        resultado_final=list(G.nodes())
        #resultado_final = reconstruct_path(busqueda, start_node, end_node)
        return resultado_final
